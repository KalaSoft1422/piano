...
void Piano::pianoInitialSetup(android_app *papp)
{       glUseProgram(programId);
        app=papp;
        PianoLog("Piano Constructor ");
...
void Piano::setUpAudio()
{
    loadSounds(app,audio,totLen);
    audioEngine.setUp();
}
void Piano::updateMsgsOnAction(int noOfMsgs,int pointerId)
{
    if(noOfMsgs==1)
    {//action up or down
...
int32_t Piano::onAction(int pointerId,float touchX,float touchY,PianoActionType actionType)
{
...
void Piano::setDimensions(int noKeysVisible)
{
...
void Piano::onNonKeyAction(int pointerId,float touchX, float touchY, PianoActionType actionType)
{
...
void Piano::setBlackKeyTranslations()
{
...
bool Piano::onPlay(void *audioData, int32_t numFrames)
{
...
bool Piano::onPlayNew(void *audioData, int32_t numFrames)
{
    outBuf=(int16_t *)audioData;
    static int id;//id is key or soundNo
    updatePlaylistsFromMidiMsgs();
...
void Piano::updatePlaylistsFromMidiMsgs()
{
    if (pthread_mutex_lock(&msgLock) == 0)
    {
...
void Piano::updatePlayList(int keyNo,int index,UpdateKey updateType)
{
