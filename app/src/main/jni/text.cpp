...
void TextEngine::getPixamapFromString(const char *string, PixaMap *pixaMap)
{
    ...
    pixaMap->pixels=(uint8_t *)malloc(totalStringWidth * (stringHeight) * sizeof(uint32_t));//stringHeight+1 cuz -topbearing in docs
    if(pixaMap->pixels != NULL)
    {
        pixaMap->width=totalStringWidth;pixaMap->height=stringHeight;
        textLog("The totla width and height are %d and %d", pixaMap->width, pixaMap->height);
...
status TextEngine::setTextSize(int width, int height)
{
  ...
  FT_Error  fterror =FT_Set_Char_Size(fontFace,0*64,height*64,displayParams.xdpi,displayParams.ydpi);// FT_Set_Pixel_Sizes(fontFace, 0, 64);
    if(fterror)
    {
